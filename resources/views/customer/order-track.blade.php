...
<section class="py-5 trackcode">
    <div class="container">
        <div class="card track-card">
            ...
            <!-- ORDER STATUS -->
            @php
    $currentStatusId = $masterOrder->order_status_id;
@endphp

<div class="status-bar">
    @foreach($statuses as $status)
        @php
            $done = $status->id <= $currentStatusId;

            // dynamic date mapping
            $date = match($status->order_status) {
        ...
        <div class="status-step {{ $done ? 'done' : '' }}">
            <div class="icon">
                <i class="bi {{ $icons[$status->order_status] ?? 'bi-circle' }}"></i>
            </div>

            <strong>{{ $status->order_status }}</strong>
